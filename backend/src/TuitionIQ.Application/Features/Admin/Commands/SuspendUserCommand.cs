...
public sealed record SuspendUserCommand(Guid ActorUserId, Guid TargetUserId) : IRequest;

public sealed class SuspendUserCommandHandler : IRequestHandler<SuspendUserCommand>
{
  private const string SuspensionDuration = "87600h";
  ...
  public async Task Handle(SuspendUserCommand request, CancellationToken cancellationToken)
  {
    ...
    await _supabaseAdminClient.BanUserAsync(targetUser.Id, SuspensionDuration, cancellationToken);
    ...
    _auditLogService.Add(new AuditLog(Guid.NewGuid(), "user.suspended", "users", now)
    {
      ...
      {
        ["is_active"] = false,
        ["ban_duration"] = SuspensionDuration
      }
    });
